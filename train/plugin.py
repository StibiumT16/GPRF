...
class RetrievalNDCGORM(ORM):
    def __init__(self):
        self.e5_url =  "http://127.0.0.1:11451/retrieve"
        self.bm25_url = "http://127.0.0.1:11452/retrieve"
        
    def __call__(self, completions: list[list[dict[str, str]]], original_query: list[str], qrels: list[str], retriever, **kwargs):
        import pytrec_eval, requests
        #contents = [completion[0]["content"] for completion in completions]
        qrels = dict([(str(i), eval(item)) for i, item in enumerate(qrels)])
        evaluator = pytrec_eval.RelevanceEvaluator(qrels, {'ndcg_cut.10', 'recall_100'})

        run = {}
        for i, (query, model) in enumerate(zip(completions, retriever)):
            payload = {
                "queries": [query],
                "topk": 100,
                "return_scores": True
            }
            if 'bm25' in model.lower():
                response = requests.post(self.bm25_url, json=payload)
            elif 'e5' in model.lower():
                response = requests.post(self.e5_url, json=payload)
            else:
            ...
            run[str(i)] = dict([(it['document']['id'], it['score']) for it in response.json()[0]])
        
        results = evaluator.evaluate(run)
        
        rewards = [results[str(i)]['ndcg_cut_10'] + 0.2 * results[str(i)]['recall_100'] for i in range(len(completions))]
...
orms['external_ndcg'] = RetrievalNDCGORM
